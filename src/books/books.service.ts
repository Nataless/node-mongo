...
export class BooksService {
  ...
  async getBook(id: string): Promise<BookDocument> {
    return this.BookModel.findById({ _id: id }).exec() as any;
  }

  async deleteBook(id: string): Promise<BookDocument> {
    return this.BookModel.findByIdAndDelete({ _id: id }).exec() as any;
  }

  async updateBook(id: string, data: CreateBookDto): Promise<BookDocument> {
    return this.BookModel.findOneAndUpdate({ _id: id }, data).exec() as any;
  }
}
